          ...
          expenses.map(exp => (
            <div key={exp.id} className='w-full flex justify-center items-center m-3'>
              <div className='w-1/3 flex justify-center items-center bg-blue-200 min-w-[250px] p-3 rounded-md'>
                ...
                <Icon onClick={() => deleteExpense(exp.id)} className='hover:scale-105 hover:rotate-6 duration-150 text-white cursor-pointer relative right-0 pl-2 text-[30px] z-10' icon="ph:trash-fill" />
              </div>
            </div>
          ))
        }
